...
class OracleRunner(LoggingRunner):
    """Sirve las evaluaciones desde un CSV completo en vez de correr el solver.

    Para RE-JUGAR una corrida especulativa contra la matriz ya pagada: el
    motor pide `(s, pi)` de a 15 y de a 25 como siempre, y su contabilidad
    (`count`, `total_runs`, trayectoria) es identica a una corrida real -- pero
    cada pedido se resuelve con una busqueda en el cache.  Esto NO es lo que
    hace `resume`: `resume` precarga todo y el motor lo ve completo de entrada.

    Un par que no esta en el oraculo es un cache MISS: con `strict=True` se
    lanza (util mientras haya un run vivo escribiendo el CSV); con
    `strict=False` se corre el solver de verdad y se registra como tal.  Los
    misses son la medida de "cuanto se desvio del camino ya explorado".
    """

    def __init__(self, *a, oracle_csv: str, strict: bool = True, **kw):
        super().__init__(*a, **kw)
        self.strict = strict
        self.oracle: dict = {}
        with open(oracle_csv, newline="") as fh:
            for row in csv.DictReader(fh):
                if row["status"] != "ok" or not row["value"]:
                    continue
                k = (row["strategy"], row["theta"], row["instance"])
                self.oracle.setdefault(k, float(row["value"]))   # el primero gana
        self.hits = 0
        self.misses = 0

    def _evaluate(self, s: Strategy, instance: str) -> float:
        theta = ";".join(f"{k}={v}" for k, v in sorted(s.theta.items()))
        v = self.oracle.get((s.name, theta, instance))
        if v is not None:
            self.hits += 1
            with self._lock:
                self._w.writerow([f"{time.time() - self._t0:.1f}", s.name, theta,
                                  instance, default_family(instance),
                                  f"{v:.6f}", "0.00", "cache", ""])
            return v
        self.misses += 1
        if self.strict:
            raise RuntimeError(f"cache MISS: {s.params} en {instance[-40:]}")
        return super()._evaluate(s, instance)      # solver real, status 'ok'
