...
for f in sorted(glob.glob(os.path.join(HERE, "stage3_replay_*_result.json")) + glob.glob(os.path.join(HERE, "stage3_var_*_result.json"))):
    ...
    if tag.startswith("replay_calr_") or tag.startswith("cal_"): continue   # calibracion: otro orden/umbral, no va a la tabla        # el motor original quedo descartado; sus 20 semillas no van a la tabla
