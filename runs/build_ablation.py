...
for f in sorted(glob.glob(os.path.join(HERE, "stage3_replay_*_result.json")) + glob.glob(os.path.join(HERE, "stage3_var_*_result.json"))):
    ...
    if any(tag.startswith(x) for x in ("calr_","cal_","ab_out_","ab_auto_","regress","smoke")): continue   # calibracion: otro orden/umbral, no va a la tabla        # el motor original quedo descartado; sus 20 semillas no van a la tabla
